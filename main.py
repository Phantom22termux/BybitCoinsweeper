...
class ByBit:
    ...
    def main(self):
        ...
        while True:
            ...
            menu_table.add_row(Rule(style="cyan"))
            menu_table.add_row("===== LORDBEAST'S PARADISE =====")
            menu_table.add_row(Rule(style="cyan"))
            menu_table.add_row("[green]1. ADD Query[/green]")
            menu_table.add_row("[green]2. RESET Query [/green]")
            menu_table.add_row("[green]3. START Coinsweeper[/green]")
            menu_table.add_row("[red]4. ABORT[/red]")
            menu_table.add_row(Rule(style="cyan"))
            
            console.print(Panel(Align.center(menu_table), border_style="cyan", box=ROUNDED, width=60))
            
            choice = animated_input("Please Enter Your Command: ")
            ...
            if choice == '1':
                ...
                secret_code = animated_input("Enter the KEY: ")
                if secret_code == "BEASTOP":
                    self.start_process()
                else:
                    self.log("Invalid KEY. Access denied. Join t.me/lordbeastop to get KEY....", "ERROR")
            elif choice == '4':
